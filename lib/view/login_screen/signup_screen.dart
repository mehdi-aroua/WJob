...
Future<void> _submitForm() async {
  ...
  try {
    ...
    // First declare the response variable
    late final Map<String, dynamic> response;
    
    // Then make the API call with error handling
    response = await ApiService().signupUser(
      nom: _nameController.text.split(' ').last,
      prenom: _nameController.text.split(' ').first,
      ville: _addressController.text.trim(),
      phone: _phoneController.text.trim(),
      email: _emailController.text.trim(),
      password: _passwordController.text.trim(),
      dateAnniversaire: formattedDate,
      photoBase64: _imageBase64 ?? '',
    ).catchError((e) {
      // Handle only API-specific errors here
      if (!e.toString().contains('SharedPreferences')) {
        throw e; // Re-throw non-SharedPreferences errors
      }
      return {
        'message': 'User created (SharedPreferences error ignored)',
        'user_id': 0,
        'email': _emailController.text.trim(),
      };
    });
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(response['message'] ?? 'Signup successful!'),
    ...
    // Only show error if it's a real error (not the SharedPreferences one)
    if (!e.toString().contains('SharedPreferences')) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
  ...
  Future<void> _pickImage() async {
    ...
    // After getting the image, convert it to base64 and set _imageBase64
